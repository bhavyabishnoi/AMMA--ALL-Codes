...
# reading dates with other than default format
# d - Day e.g 1, 2 etc
# m - month
# b - month /Jan, Feb
# B - Month January
...
csv_file <- read.table(file="C:\\Ram\\General 20150804 v1\\Trainings\\R Programming for Data Science\\data\\binary.csv",
                                header = TRUE, 
                                sep = ',')
csv_file<-read.table("binary.csv",header=T,sep=",")
...
flipkartPage <-getPage(page="flipkart",
                       token=accessToken,
                       n=150)
matches_url<- getURL("https://en.wikipedia.org/wiki/India%E2%80%93Pakistan_cricket_rivalry")
